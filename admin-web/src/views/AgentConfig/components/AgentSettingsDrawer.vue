<template>
  <el-drawer
    ...
    :title="purposeLabel || 'DeepSeek Agent 设置'"
    size="560px"
    destroy-on-close
  ...
  >
    <div v-loading="loading" class="settings-panel">
      <div class="settings-status">
        <div>
          <div class="status-title">{{ purposeLabel || 'DeepSeek' }}</div>
      ...
      <el-form label-width="118px" class="settings-form">
        ...
        <el-form-item label="复核模型">
          <el-input v-model="form.reviewModel" placeholder="deepseek-v4-pro" />
          <div class="form-tip">用于营养校验、复杂来源与复核，建议 Pro。</div>
        </el-form-item>
        ...
        <el-form-item label="API Key">
          ...
          />
          <div class="form-tip">后端只保存加密密钥；页面不会回显完整 API Key。</div>
        </el-form-item>

        <el-form-item label="最大并发">
          <el-input-number v-model="form.maxConcurrency" :min="1" :max="5" controls-position="right" />
        </el-form-item>

        <el-form-item label="超时毫秒">
          <el-input-number v-model="form.requestTimeoutMs" :min="5000" :max="300000" :step="5000" controls-position="right" />
        </el-form-item>

        <el-form-item label="重试次数">
          <el-input-number v-model="form.retryCount" :min="0" :max="5" controls-position="right" />
        </el-form-item>
      </el-form>
    </div>
    ...
    <template #footer>
      <div class="drawer-actions">
        ...
        <el-button :disabled="!settings?.apiKeyConfigured" :loading="saving" @click="emitClearKey">
          清除密钥
        </el-button>
        <el-button :loading="testing" :disabled="saving" @click="emit('test', buildPayload())">
          测试连接
        </el-button>
        <el-button type="primary" :loading="saving" :disabled="loading || testing" @click="emit('save', buildPayload())">
          保存设置
        </el-button>
      </div>
    </template>
...
const apiKeyStatus = computed(() => {
  if (!props.settings?.apiKeyConfigured) return 'API Key 未配置'
