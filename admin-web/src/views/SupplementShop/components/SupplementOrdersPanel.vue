<template>
  <div class="supplement-orders-panel" :class="{ 'is-embedded': embedded }">
    <div class="page-header">
      <div>
        <!-- 嵌入「订单管理」的标签页时，外层标签已经写了「补剂订单」，再放大标题就是重复信息 -->
        <h2 v-if="!embedded">补剂订单</h2>
        <p class="page-desc">
...
/**
 * embedded=true 表示本组件被「订单管理」的补剂订单标签页复用：
 * 只改页面级外壳（页边距 / 重复标题），业务逻辑与独立访问时完全一致，
 * 保证两条入口不会各自演化出不同行为。
 */
...
/** 打印标签是独立路由；嵌入标签页访问时带上来源，标签页的「返回」才知道要回到补剂标签 */
function labelsLocation(orderId: string) {
  return props.embedded
    ? { path: `/supplement-shop/orders/${orderId}/labels`, query: { from: 'orders' } }
    : `/supplement-shop/orders/${orderId}/labels`;
}
...
async function submitPack() {
  ...
  try {
    ...
    try {
      ...
      router.push(labelsLocation(updated.id));
    } catch {
...
function openLabels(order: SupplementOrder) {
  router.push(labelsLocation(order.id));
}
...
<style scoped>
.supplement-orders-panel {
  padding: 20px;
}

/* 嵌进「订单管理」的标签页时，外层已经提供了内容区留白，这里再留一次会显得上下都是空的 */
.supplement-orders-panel.is-embedded {
  padding: 0;
}

.supplement-orders-panel.is-embedded .page-header {
  margin-top: 4px;
}
