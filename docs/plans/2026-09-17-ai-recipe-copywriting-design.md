...
## 九、⚠️ 实施中发现的前置依赖（必须先解决）

### 9.1 顾客端筛选接口必须过滤空标签与分组标签

**问题（实测复现）**：把 38 个词表标签落库后，顾客端筛选接口 `/recipes/filter-options`
立刻返回 **43 个标签，其中 40 个关联为 0**，并且**包含了「原料事实」这类分组名**。
线上小程序会因此显示一大堆"点了没结果"的空筛选项。

**根因**：`filter-options` 当前返回**全部**标签，不做任何过滤。

**必须先做的后端改动**：
1. 只返回**叶子标签**（`parentId` 为空的分组标签不参与筛选）
2. 只返回**至少关联 1 个公开食谱**的标签（count > 0）

**处置记录（2026-09-17）**：词表已 seed 到生产后**已回滚**（删除 40 个空标签），
生产恢复为 `低脂 / 易消化 / 挑食友好` 三个标签。待上述后端改动上线后，再重新执行 seed。

### 9.2 实施顺序修正

| 步骤 | 内容 | 说明 |
|:---:|------|------|
| 0 | **后端：`filter-options` 过滤空标签与分组标签** | 新增前置项，必须先做 |
| 1 | 重新 seed 合规标签词表 | 依赖步骤 0 |
| 2~8 | 原第七节步骤（字段 / AI 服务 / 接口 / 后台 / 校验 / 详情页） | — |

### 9.3 另一个待确认的设计点

原设计稿的「适用对象」层包含 `幼犬期 / 成犬维持 / 老年犬`，
但食谱**已有独立的生命阶段字段与筛选器**（实测 10 个选项正常工作）。
若再建同名健康标签，会造成**重复筛选与计数口径混乱**。

**本次实施已从词表中移除这三个标签**（AI 生成卖点时仍可从生命阶段字段取用）。
如你希望保留，请告知。
