...
### 阶段三（2026-09-26 完成 · 提交 4f765b4d）

试吃装那批提交、工作区干净后开工，按已确认的三个口径落地：

| 口径 | 落地方式 |
|---|---|
| 货款端直接减 | 订单金额校验规则 **`货款 + 运费 = 总额` 完全不变**，退款/结算/财务报表沿用现有逻辑 |
| 只抵货款不抵运费 | 预览与下单都只减 `amountProduct`；实测运费 ¥37.18 一分未减 |
| 余额结转 | 货款不足只抵剩下的；实测 ¥300 额度分两单用掉（172.65 + 127.35） |

**额度台账**（先做，见下）
**下单链路**：预览与下单两条路径都接入，且**下单以快照为准不重算**（避免页面价与实付价错位）；顺序是"**先扣额度、再落订单**"，落单失败会把额度还回去；抵扣查询失败降级为不抵扣，不拦住报价。
**数据结构**：`order` 表加 `credit_amount_applied` 与 `custom_recipe_credit_order_id`（仅展示与审计，不参与金额计算；来源单**不加唯一约束**以支持余额结转）。
**展示**：小程序订购页底栏、结算页金额明细、成品订单详情；后台订单详情。

**修掉一个集成缺陷**：定制单上存的是**食谱主键**（外键指向 `recipe.id`），而下单链路传的是**业务食谱号**（`recipe.recipeId`），两者原本永远匹配不上、抵扣会静默失效。现在先归一化再匹配。

**验证**：后端 246 套 / 2196 项、小程序 88 套 / 877 项全部通过；`build:mp-weixin` 成功；真库实测额度扣减、余额结转、订单留痕与详情接口返回均正确。
