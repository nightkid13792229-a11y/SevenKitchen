<template>
  <view class="home-container">
    ...
    <view v-if="showMigrationOverlay" class="migration-overlay">
      <view class="migration-dialog">
        <view class="migration-dialog-header">
          <view>
            <text class="migration-dialog-kicker">资料迁移提醒</text>
            <text class="migration-dialog-title">旧版资料迁移确认</text>
          </view>
          <text class="migration-dialog-close" @tap="closeMigrationOverlay">×</text>
        </view>

        <view v-if="!migrationSubmitted">
          <text class="migration-dialog-desc">
            旧版小程序仍可先浏览首页。若你需要把旧版资料同步到新版，请填写手机号作为身份核验线索。手机号仅用于旧版账号核验和资料迁移确认，不用于营销或其他无关用途。
          </text>

          <view class="migration-dialog-steps">
            <text class="migration-dialog-step">1. 在旧版登录账号并填写手机号</text>
            <text class="migration-dialog-step">2. 打开微信搜索新版小程序：赛文的食堂</text>
            <text class="migration-dialog-step">3. 在新版授权同一个手机号并二次确认同步</text>
          </view>

          <view v-if="isLoggedIn" class="migration-form">
            <view class="migration-form-item">
              <text class="migration-form-label">手机号</text>
              <input
                class="migration-form-input"
                type="number"
                maxlength="11"
                v-model="migrationPhone"
                placeholder="请输入手机号"
              />
            </view>
            <view class="migration-form-item">
              <text class="migration-form-label">再次确认手机号</text>
              <input
                class="migration-form-input"
                type="number"
                maxlength="11"
                v-model="migrationPhoneConfirm"
          ...
          <button
            v-if="isLoggedIn"
            class="migration-primary-btn"
            :disabled="migrationSubmitting"
            @tap="submitMigrationPhone"
          >
            {{ migrationSubmitting ? "提交中..." : "提交手机号" }}
          </button>
          <button v-else class="migration-primary-btn" @tap="goToLogin">
            先登录旧版账号
          </button>
          <button class="migration-secondary-btn" @tap="closeMigrationOverlay">
            稍后处理
          </button>
        </view>

        <view v-else class="migration-result">
          <text class="migration-result-title">手机号已提交</text>
          <text class="migration-result-desc">
            已记录手机号{{ submittedMaskedPhone ? " " + submittedMaskedPhone : "" }}。请打开微信搜索“赛文的食堂”，进入新版后授权同一个手机号并确认是否同步。
          </text>
          <button class="migration-primary-btn" @tap="copyNewMiniappName">
            复制新版小程序名称
          </button>
          <button class="migration-secondary-btn" @tap="closeMigrationOverlay">
            我知道了
...
import { copyNewMiniappName as copyLegacyMiniappName } from '../../utils/legacy-migration'
...
const showMigrationOverlay = ref(false)
const migrationPhone = ref('')
const migrationPhoneConfirm = ref('')
const migrationSubmitting = ref(false)
const migrationSubmitted = ref(false)
const submittedMaskedPhone = ref('')
...
// 检查登录状态
function normalizeMigrationPhone(value: string): string {
  return String(value || '').replace(/\s+/g, '').trim()
}

function isValidMigrationPhone(value: string): boolean {
  return /^1[3-9]\d{9}$/.test(value)
}

function shouldShowMigrationOverlay(): boolean {
  if (migrationSubmitted.value) {
    return false
  }

  if (uni.getStorageSync('legacyMigrationPhoneSubmittedAt')) {
    return false
  }

  const dismissedAt = uni.getStorageSync('legacyMigrationOverlayDismissedAt')
  const today = new Date().toDateString()
  if (dismissedAt === today && !uni.getStorageSync('legacyMigrationOverlayRequested')) {
    return false
  }
...
function openMigrationOverlaySoon(force = false): void {
  if (!force && !shouldShowMigrationOverlay()) {
    return
  }

  setTimeout(() => {
    if (force || shouldShowMigrationOverlay()) {
      showMigrationOverlay.value = true
      uni.removeStorageSync('legacyMigrationOverlayRequested')
    }
  }, 500)
}
...
// 跳转登录页
const goToLogin = () => {
  showMigrationOverlay.value = false
  uni.navigateTo({
...
function closeMigrationOverlay() {
  showMigrationOverlay.value = false
  uni.setStorageSync('legacyMigrationOverlayDismissedAt', new Date().toDateString())
}

function copyNewMiniappName() {
  copyLegacyMiniappName()
}

async function submitMigrationPhone() {
  const first = normalizeMigrationPhone(migrationPhone.value)
  const second = normalizeMigrationPhone(migrationPhoneConfirm.value)

  if (!isValidMigrationPhone(first)) {
  ...
  try {
    ...
    submittedMaskedPhone.value = response.data?.phone || first.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')
    migrationSubmitted.value = true
    uni.setStorageSync('legacyMigrationPhoneSubmittedAt', Date.now())
    uni.setStorageSync('legacyMigrationPhoneMasked', submittedMaskedPhone.value)
    uni.removeStorageSync('legacyMigrationOverlayDismissedAt')
  } catch (error: any) {
    uni.showToast({
      title: error?.message || '手机号提交失败，请稍后重试',
      icon: 'none',
    })
  } finally {
...
.migration-dialog-kicker,
.migration-dialog-title,
.migration-dialog-desc,
.migration-dialog-step,
.migration-form-label,
.migration-result-title,
.migration-result-desc {
  display: block;
}
...
.migration-primary-btn,
.migration-secondary-btn {
  width: 100%;
  height: 88rpx;
  margin-top: 20rpx;
  border-radius: 12rpx;
