/**
 * 精臣打印机的**分包内精简副本**，只有补剂标签打印需要的那部分。
 *
 * 与主包 utils/jcing-printer.ts 的差别：去掉了 printLabel()（那条路是画鲜食
 * 标签用的，会连带拉进 36KB 的 label-renderer），补剂走的是 printLabelFromImage。
 * 放在分包里是为了不让每个顾客首次打开小程序就下载精臣 SDK（180KB+）。
 */
/**
 * 精臣打印机封装类
 * 提供打印机连接、管理和打印功能
